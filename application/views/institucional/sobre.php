<?php
$this->load->view("commons/header");
?>

<div class="row">
  <div class="col-12 banner">
    <h1 class="titulo">QUEM SOMOS</h1>
    <h2>Introdução</h2>
    <p>
      <img class="img-intro img-fluid" src='<?=base_url("assets/images/logo-coopergac.jpg")?>' />
        <iframe class="" height="400" width="600" src="https://www.youtube.com/embed/CWb2X1abqkA" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
    </p>


    <h2>Significado</h2>
    <p>
      Lorem ipsum dolor sit amet, Lorem ipsum dolor sit amet, consectetur adipiscing elit. Pellentesque eu nisl
      augue. Proin egestas odio ac tempor vulputate. Nulla vestibulum imperdiet pretium. Sed pellentesque aliquam congue.consectetur adipiscing elit. Pellentesque eu nisl
      augue. Proin egestas odio ac tempor vulputate. Nulla vestibulum imperdiet pretium. Sed pellentesque aliquam congue.
      Lorem ipsum dolor sit amet, consectetur adipiscing elit. Pellentesque eu nisl
      augue. Proin egestas odio ac tempor vulputaLorem ipsum dolor sit amet, consectetur adipiscing elit. Pellentesque eu nisl
      augue. Proin egestas odio ac tempor vulputate. Nulla vestibulum imperdiet pretium. Sed pellentesque aliquam congue.te. Nulla vestibulum imperdiet pretium. Sed pellentesque aliquam congue.
      Lorem ipsum dolor sit amet, consectetur adipiscing elit. Pellentesque eu nisl
      augue. Proin egestas odio ac tempor vulputate. Nulla vestibulum imperdiet pretium. Sed pellentesque aliquam congue.
      Lorem ipsum dolor sit amet, consectetur adipiscing elit. Pellentesque eu nisl
      augue. Proin egestas odio ac tempor vulputate. Nulla vestibulum imperdiet pretium. Sed pellentesque aliquam congue.
    </p>

    <h2>Objetivos</h2>
    <p>
      Lorem ipsum dolor sit amet, Lorem ipsum dolor sit amet, consectetur adipiscing elit. Pellentesque eu nisl
      augue. Proin egestas odio ac tempor vulputate. Nulla vestibulum imperdiet pretium. Sed pellentesque aliquam congue.consectetur adipiscing elit. Pellentesque eu nisl
      augue. Proin egestas odio ac tempor vulputate. Nulla vestibulum imperdiet pretium. Sed pellentesque aliquam congue.
      Lorem ipsum dolor sit amet, consectetur adipiscing elit. Pellentesque eu nisl
      augue. Proin egestas odio ac tempor vulputaLorem ipsum dolor sit amet, consectetur adipiscing elit. Pellentesque eu nisl
      augue. Proin egestas odio ac tempor vulputate. Nulla vestibulum imperdiet pretium. Sed pellentesque aliquam congue.te. Nulla vestibulum imperdiet pretium. Sed pellentesque aliquam congue.
      Lorem ipsum dolor sit amet, consectetur adipiscing elit. Pellentesque eu nisl
      augue. Proin egestas odio ac tempor vulputate. Nulla vestibulum imperdiet pretium. Sed pellentesque aliquam congue.
      Lorem ipsum dolor sit amet, consectetur adipiscing elit. Pellentesque eu nisl
      augue. Proin egestas odio ac tempor vulputate. Nulla vestibulum imperdiet pretium. Sed pellentesque aliquam congue.
    </p>
  </div>
</div>
  <div class="row">
    <div class="col-md-6">
      <img src="<?=base_url('assets/images/pilares.jpg')?>" alt="banner coopergac" class="img-fluid absolute">

      <div class="hero-content pilares-banner d-none d-lg-block">
        <i class="fas fa-book" style="font-size:60px"></i>
        <h3>Educação</h3>

        <i class="fas fa-tree" style="font-size:60px"></i>
        <h3>Sustentabilidade</h3>

        <i class="fas fa-hands-helping" style="font-size:60px"></i>
        <h3>Respeito</h3>
      </div>

      <div class="hero-content pilares-banner d-none d-md-block d-lg-none">
        <i class="fas fa-book" style="font-size:60px; padding-top:1%;"></i>
        <h3 style="padding-bottom:1%;">Educação</h3>

        <i class="fas fa-tree" style="font-size:60px; padding-top:1%;"></i>
        <h3 style="padding-bottom:1%;">Sustentabilidade</h3>

        <i class="fas fa-hands-helping" style="font-size:60px; padding-top:1%;"></i>
        <h3 style="padding-bottom:50%;margin-bottom:500px;">Respeito</h3>
      </div>

      <div class="hero-content pilares-banner d-sm-block d-md-none ">
        <i class="fas fa-book" style="font-size:60px; padding-top:1%;"></i>
        <h3 style="padding-bottom:10%;">Educação</h3>

        <i class="fas fa-tree" style="font-size:60px; padding-top:10%;"></i>
        <h3 style="padding-bottom:10%;">Sustentabilidade</h3>

        <i class="fas fa-hands-helping" style="font-size:60px; padding-top:10%;"></i>
        <h3 style="padding-bottom:10%;">Respeito</h3>
      </div>


    </div>
    <div class="col-md-6">
      <h2>Pilares</h2>
      <p>
        consectetur adipiscing elit. Pellentesque eu nisl
        augue. Proin egestas odio ac tempor vulputaLorem ipsum dolor sit amet, consectetur adipiscing elit. Pellentesque eu nisl
        augue. Proin egestas odio ac tempor vulputate. Nulla vestibulum imperdiet pretium. Sed pellentesque aliquam congue.te. Nulla vestibulum imperdiet pretium. Sed pellentesque aliquam congue.
        Lorem ipsum dolor sit amet, consectetur adipiscing elit. Pellentesque eu nisl
        augue. Proin egestas odio ac tempor vulputate. Nulla vestibulum imperdiet pretium. Sed pellentesque aliquam congue.
        Lorem ipsum dolor sit amet, consectetur adipiscing elit. Pellentesque eu nisl
        augue. Proin egestas odio ac tempor vulputate. Nulla vestibulum imperdiet pretium. Sed pellentesque aliquam congue.
      </p>
      <h5>Educação</h5>
      <p>
        Lorem ipsum dolor sit amet, Lorem ipsum dolor sit amet, consectetur adipiscing elit. Pellentesque eu nisl
        augue. Proin egestas odio ac tempor vulputate. Nulla vestibulum imperdiet pretium.
      </p>
      <h5>Sustentabilidade</h5>
      <p>
        Lorem ipsum dolor sit amet, Lorem ipsum dolor sit amet, consectetur adipiscing elit. Pellentesque eu nisl
        augue. Proin egestas odio ac tempor vulputate. Nulla vestibulum imperdiet pretium.
        Lorem ipsum dolor sit amet, Lorem ipsum dolor sit amet, consectetur adipiscing elit. Pellentesque eu nisl
        augue. Proin egestas odio ac tempor vulputate. Nulla vestibulum imperdiet pretium.
      </p>
      <h5>Respeito</h5>
      <p>
        Lorem ipsum dolor sit amet, Lorem ipsum dolor sit amet, consectetur adipiscing elit. Pellentesque eu nisl
        augue. Proin egestas odio ac tempor vulputate. Nulla vestibulum imperdiet pretium.
      </p>
      <br />
      <a href="<?=base_url('contato')?>" class="btn btn-success btn-lg btn-center"> Fale Conosco </a>
    </div>
  </div>

<?php
$this->load->view("commons/footer");
?>
